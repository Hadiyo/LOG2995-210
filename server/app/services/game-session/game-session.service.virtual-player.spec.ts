...
describe('GameSessionService virtual players', () => {
    ...
    it('opens a closed adjacent door and then continues through it on later virtual decisions', () => {
        const serviceInternals = harness.getServiceInternals();
        const randomSpy = jest.spyOn(Math, 'random').mockReturnValue(0);
        const runtime = makeRuntime({
            match: makeMatch({
                map: [
                    { position: { x: 0, y: 0 }, tileType: TileType.DIRT, isWalkable: true, isOccupied: false },
                    { position: { x: 1, y: 0 }, tileType: TileType.DOOR, isWalkable: false, isOccupied: false },
                    { position: { x: 2, y: 0 }, tileType: TileType.DIRT, isWalkable: true, isOccupied: false },
                ],
                players: [
                    makeMatchPlayer({
        ...
        expect(runtime.match.map.find((cell) => cell.position.x === 1 && cell.position.y === 0)?.isWalkable).toBe(true);
        expect(runtime.turnState.actionTaken).toBe(true);
        expect(runtime.match.players.find((player) => player.id === 'player-1')?.position).toEqual({ x: 0, y: 0 });
    ...
    it('moves toward a closed door before opening it when the target is on the other side', () => {
        const serviceInternals = harness.getServiceInternals();
        const randomSpy = jest.spyOn(Math, 'random').mockReturnValue(0);
        const runtime = makeRuntime({
            match: makeMatch({
                map: [
                    { position: { x: 0, y: 0 }, tileType: TileType.DIRT, isWalkable: true, isOccupied: false },
                    { position: { x: 1, y: 0 }, tileType: TileType.DIRT, isWalkable: true, isOccupied: false },
                    { position: { x: 2, y: 0 }, tileType: TileType.DOOR, isWalkable: false, isOccupied: false },
                    { position: { x: 3, y: 0 }, tileType: TileType.DIRT, isWalkable: true, isOccupied: false },
                ],
                players: [
                    makeMatchPlayer({
                        id: 'player-1',
        ...
        expect(runtime.match.players.find((player) => player.id === 'player-1')?.position).toEqual({ x: 1, y: 0 });
        expect(runtime.match.map.find((cell) => cell.position.x === 2 && cell.position.y === 0)?.isWalkable).toBe(false);
        randomSpy.mockRestore();
    });
