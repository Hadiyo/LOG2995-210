...
import { MapValidationIssue, STARTS_REQUIRED_BY_SIZE } from './map-validation.types';
...
const getNeighborKeys = (position: Vec2): string[] => [
    positionKey(position.x - 1, position.y),
    positionKey(position.x + 1, position.y),
    positionKey(position.x, position.y - 1),
    positionKey(position.x, position.y + 1),
];
...
const addNameIssues = (map: EditorMap, issues: MapValidationIssue[]): void => {
    if (!isNonEmpty(map.name)) {
        issues.push({ code: 'NAME_REQUIRED', message: 'Le nom de la carte est requis.' });
    }
};

const addDescriptionIssues = (map: EditorMap, issues: MapValidationIssue[]): void => {
    if (!isNonEmpty(map.description)) {
        issues.push({ code: 'DESCRIPTION_REQUIRED', message: 'La description de la carte est requise.' });
    }
};
...
const getBlockedDoorwayPositions = (cellsByKey: Map<string, Cell>, blockedKeys: Set<string>): Vec2[] => {
    ...
    for (const cell of cellsByKey.values()) {
        ...
        } else if (blockedKeys.has(positionKey(x - 1, y)) && blockedKeys.has(positionKey(x + 1, y))) {
            blocked.push({ x, y });
        }
    }
...
const addFlagOnDoorIssues = (map: EditorMap, cellsByKey: Map<string, Cell>, issues: MapValidationIssue[]): void => {
    const invalidFlags = map.objects
        ...
        .filter((object) => cellsByKey.get(positionKey(object.position.x, object.position.y))?.tileType === TileType.DOOR)
        .map((object) => object.position);
...
const addStartOnOpenDoorIssues = (map: EditorMap, cellsByKey: Map<string, Cell>, issues: MapValidationIssue[]): void => {
    const invalidStarts = map.objects
        ...
        .filter((object) => {
            const cell = cellsByKey.get(positionKey(object.position.x, object.position.y));
...
const collectUnreachablePositions = (traversable: Set<string>, reachable: Set<string>, cellsByKey: Map<string, Cell>): Vec2[] => {
...
export const createMapValidationIssues = (map: EditorMap): MapValidationIssue[] => {
